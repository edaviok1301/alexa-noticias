...
SECCION     = "Tecnologia"
NOMBRE_JSON = "noticias_tecnologia.json"
...
def _formato_alexa(noticias):
    ...
    for i, n in enumerate(noticias, start=1):
        items.append({
            "uid": f"tecnologia-{i:03d}",
            "updateDate": ahora,
            "titleText": "Tecnología",
            "mainText": f"Tecnología. {n['titulo']}. {n['resumen']}",
...
def generar(ec_noticias, rpp_noticias):
    ...
    for fn in [larepublica.obtener_noticia, peru21.obtener_noticia]:
        try:
            ...
            print(f"[Tecnologia] Error en scraper: {e}")
